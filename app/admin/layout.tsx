...
import { LayoutDashboard, Users, Ticket, LogOut, Scissors } from "lucide-react"
...
export default function AdminLayout({
    ...
    return (
        <div className="min-h-screen bg-neutral-950 flex font-sans text-white">
            {/* Sidebar */}
...
function NavItem({ href, icon, label, active }: { href: string, icon: React.ReactNode, label: string, active: boolean }) {
    return (
        <Link href={href}>
            <Button
                variant="ghost"
                className={`w-full justify-start ${active ? "bg-white/10 text-yellow-500" : "text-neutral-400 hover:text-white hover:bg-white/5"}`}
            >
                {icon}
