        ...
        <div className="space-y-8">
            <div className="flex justify-between items-end">
                <div>
                    ...
                    <p className="text-neutral-400 mt-1">Visão geral e faturamento da plataforma</p>
                </div>
                ...
                <Dialog open={isExpensesModalOpen} onOpenChange={setIsExpensesModalOpen}>
                    <DialogContent className="bg-[#1c1c1c] border-white/5 text-white max-w-md">
                        ...
                        <div className="space-y-4 pt-4">
                            ...
                            <div className="border border-zinc-800 rounded-md overflow-hidden bg-black/20">
                                <Table>
                                    <TableHeader>
                                    ...
                                    <TableBody>
                                        {expenses.length === 0 ? (
                                            <TableRow>
                                                <TableCell colSpan={3} className="text-center py-6 text-neutral-500">Nenhuma despesa cadastrada</TableCell>
                                            </TableRow>
                                        ) : expenses.map(exp => (
                            ...
                            <div className="pt-2 flex justify-between items-center bg-black/40 px-4 py-3 rounded-md border border-zinc-800/50">
                                <span className="text-sm font-medium text-neutral-400">Total de Custos</span>
            ...
            {!loading && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pt-4">
                            ...
                            <div className="h-[300px] w-full mt-4">
                                <ResponsiveContainer width="100%" height="100%">
                                    <LineChart data={chartData.revenue}>
                                        ...
                                        />
                                        <Line type="monotone" dataKey="faturamento" stroke="#DBC278" strokeWidth={3} dot={{ fill: "#DBC278", strokeWidth: 2, r: 4 }} activeDot={{ r: 6 }} />
                                    </LineChart>
                                </ResponsiveContainer>
                            </div>
                        </CardContent>
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={chartData.bookings} layout="vertical" margin={{ left: 0, right: 30 }}>
                                        ...
                                        <Tooltip
                                            ...
                                            cursor={{ fill: '#2a2a2a' }}
                                        />
                                        <Bar dataKey="agendamentos" fill="#DBC278" radius={[0, 4, 4, 0]} barSize={24}>
                                            <LabelList dataKey="agendamentos" position="right" fill="#DBC278" fontSize={12} className="font-bold" />
                                        </Bar>
                                    </BarChart>
                                </ResponsiveContainer>
                            </div>
                    ...
                    <Card className="bg-[#1c1c1c] border-white/5 text-white lg:col-span-2">
                        <CardHeader>
                            <CardTitle className="text-lg font-medium flex items-center gap-2">
                                ...
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={chartData.shops}>
                                        ...
                                        <Tooltip
                                            ...
                                            cursor={{ fill: '#2a2a2a' }}
                                        />
                                        <Bar dataKey="barbearias" fill="#DBC278" radius={[4, 4, 0, 0]} barSize={40}>
                                            <LabelList dataKey="barbearias" position="top" fill="#DBC278" fontSize={12} className="font-bold" />
                                        </Bar>
                                    </BarChart>
                                </ResponsiveContainer>
                            </div>
...
function StatCard({ title, value, icon, desc, onClick }: { title: string, value: string | number, icon: React.ReactNode, desc?: string, onClick?: () => void }) {
    return (
        <Card
            className={`bg-[#1c1c1c] border-white/5 text-white relative overflow-hidden ${onClick ? 'cursor-pointer hover:border-white/20 hover:bg-white/5 transition-all' : ''}`}
            onClick={onClick}
        >
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                <div className="z-10 bg-black/20 p-2 rounded-md">{icon}</div>
            </CardHeader>
            ...
            <div className="absolute -right-4 -bottom-4 w-24 h-24 bg-white/5 rounded-full blur-2xl z-0 pointer-events-none" />
        </Card>
    )
}
