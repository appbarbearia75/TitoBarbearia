...
export default function AnalyticsPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            let totalRevenue = 0 // For current month
            let totalBookings = 0 // For current month
            ...
            // Monthly Revenue Map (for the chart history)
            ...
            bookings?.forEach(booking => {
                ...
                // Check if this booking is in the CURRENT MONTH
                const isCurrentMonth = bookingDate.getMonth() === currentMonthIdx && bookingDate.getFullYear() === currentYearIdx

                if (isCompleted) {
                    const monthKey = format(bookingDate, 'MMM', { locale: ptBR })
                    const monthKeyCap = monthKey.charAt(0).toUpperCase() + monthKey.slice(1)

                    // Add to chart history if within the 6-month window
                    if (monthlyRevenue.has(monthKeyCap)) {
                        monthlyRevenue.set(monthKeyCap, (monthlyRevenue.get(monthKeyCap) || 0) + Number(price))
                    }

                    // Strict stats for the CURRENT MONTH cards
                    if (isCurrentMonth) {
                        totalRevenue += Number(price)
                    }
                }

                if (isNonCancelled && isCurrentMonth) {
                    totalBookings++
                }
        ...
        <div className="space-y-8 pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <Card className="bg-[#1c1c1c] border-white/5 text-white shadow-2xl shadow-black/50">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">
                            Faturamento ({format(new Date(), 'MMM', { locale: ptBR })})
                        </CardTitle>
                        <DollarSign className="h-4 w-4 text-[#DBC278]" />
                    </CardHeader>
                    <CardContent>
                ...
                <Card className="bg-[#1c1c1c] border-white/5 text-white shadow-2xl shadow-black/50">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium">
                            Agendamentos ({format(new Date(), 'MMM', { locale: ptBR })})
                        </CardTitle>
                        <Calendar className="h-4 w-4 text-[#DBC278]" />
                    </CardHeader>
                    <CardContent>
                ...
                <Card className="bg-[#1c1c1c] border-white/5 text-white shadow-2xl shadow-black/50">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        ...
                        <Users className="h-4 w-4 text-[#DBC278]" />
                    </CardHeader>
                    <CardContent>
