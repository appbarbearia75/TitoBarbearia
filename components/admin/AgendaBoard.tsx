        ...
        <div className="space-y-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-zinc-400 text-sm">Gerencie os agendamentos de hoje.</p>
                </div>
                <div className="flex items-center gap-2 bg-zinc-900 p-2 rounded-lg border border-zinc-800">
                    <button
                        onClick={() => setIsCalendarOpen(true)}
                        className="flex items-center gap-2 hover:bg-zinc-800 p-1 rounded transition-colors"
                    >
                    ...
                    <div className="flex gap-1 ml-4">
                        <Button size="sm" variant="ghost" className="h-7 px-2" onClick={() => setDate(new Date(date.setDate(date.getDate() - 1)))}>&lt;</Button>
                        <Button size="sm" variant="ghost" className="h-7 px-2" onClick={() => setDate(new Date())}>Hoje</Button>
                        <Button size="sm" variant="ghost" className="h-7 px-2" onClick={() => setDate(new Date(date.setDate(date.getDate() + 1)))}>&gt;</Button>
                    </div>
                </div>
            </div>
                            ...
                            {loading ? (
                                ...
                                bookings.map((booking) => (
                                    <TableRow key={booking.id} className="border-zinc-800 hover:bg-zinc-800/50">
                                        <TableCell className="font-mono text-lg font-bold text-yellow-500">
                                            {booking.time}
                                        </TableCell>
                                        <TableCell>
                                        ...
                                        <TableCell className="text-right">
                                            <div className="flex justify-end gap-2">
                                                {booking.status === 'pending' && (
                                                    <Button
                                                        size="sm"
                                                        className="bg-green-600/20 text-green-500 hover:bg-green-600/30"
                                                ...
                                                {booking.status === 'confirmed' && (
                                                    <Button
                                                        size="sm"
                                                        className="bg-blue-600/20 text-blue-500 hover:bg-blue-600/30"
...
function StatusBadge({ status }: { status: string }) {
    switch (status) {
        case 'confirmed':
            return <Badge className="bg-green-500/10 text-green-500 hover:bg-green-500/20 border-green-500/20">Confirmado</Badge>
        case 'cancelled':
            return <Badge className="bg-red-500/10 text-red-500 hover:bg-red-500/20 border-red-500/20">Cancelado</Badge>
        case 'completed':
            return <Badge className="bg-blue-500/10 text-blue-500 hover:bg-blue-500/20 border-blue-500/20">Concluído</Badge>
        default:
            return <Badge className="bg-yellow-500/10 text-yellow-500 hover:bg-yellow-500/20 border-yellow-500/20">Pendente</Badge>
    }
}
