...
export function WaitlistFloatingButton({ slug, onUpdate }: { slug: string, onUpdate?: () => void }) {
    ...
    return (
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
                    ...
                    <Button
                        ...
                        className={`
                            ...
                            ${hasEntries
                                ...
                                : 'bg-[#1c1c1c] hover:bg-zinc-800 text-zinc-400 border border-white/5'}
                        `}
                    >
            ...
            <SheetContent className="bg-[#09090b] border-white/5 w-full sm:max-w-md p-0 overflow-hidden flex flex-col [&>button]:opacity-100">
                <SheetHeader className="p-6 border-b border-white/5 bg-black/40">
                    <SheetTitle className="text-xl font-black text-white uppercase tracking-tighter flex items-center gap-3">
                        <div className={`w-3 h-3 rounded-full ${hasEntries ? 'bg-orange-500 animate-pulse' : 'bg-zinc-700'}`} />
                        Fila de Espera
                    </SheetTitle>
                ...
                <div className="flex-1 overflow-auto p-6 space-y-4 custom-scrollbar">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-20 px-4 text-center space-y-4 bg-white/[0.02] rounded-3xl border border-dashed border-white/5">
                            <div className="w-16 h-16 rounded-full bg-zinc-900 flex items-center justify-center">
                                <User className="w-8 h-8 text-zinc-700" />
                            </div>
                            <div className="max-w-[200px]">
                                <p className="text-zinc-200 font-bold">Ninguém aguardando</p>
                                <p className="text-xs text-zinc-600 mt-2 font-medium">Assim que algum cliente entrar na fila, ele aparecerá aqui para você.</p>
                            </div>
                        </div>
                    ) : (
                        entries.map((entry) => (
                            <div key={entry.id} className="group p-4 rounded-2xl bg-zinc-900/50 border border-white/5 hover:border-orange-500/30 hover:bg-orange-500/[0.02] transition-all duration-500 shadow-sm hover:shadow-orange-500/5">
                                <div className="flex items-start justify-between gap-4">
                                    <div className="min-w-0">
                                        <p className="text-base font-black text-white truncate capitalize tracking-tight">{entry.customer_name}</p>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button
                                            ...
                                            className="p-2 hover:bg-red-500/10 text-zinc-600 hover:text-red-500 rounded-xl transition-all duration-300"
                                            title="Remover"
                                        >
                                            <X className="w-4 h-4" />
                                        </button>
                                ...
                                <div className="flex items-center justify-between gap-4 mt-4 pt-4 border-t border-white/5">
                                    <div className="flex flex-col">
                                        <span className="text-[10px] text-zinc-600 font-black uppercase tracking-widest leading-none mb-1">Interesse</span>
                                        <div className="flex items-center gap-1.5 text-zinc-300 font-bold text-xs uppercase">
                ...
                <div className="p-6 bg-black/40 border-t border-white/5">
                    <p className="text-center text-[10px] text-zinc-700 font-black uppercase tracking-[0.2em]">Agendaê 2026</p>
                </div>
            </SheetContent>
        </Sheet>
    )
